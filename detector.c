...
float validate_detector_map(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou, const float iou_thresh, const int map_points, int letter_box, network *existing_net)
{
    ...
    for (i = nthreads; i < m + nthreads; i += nthreads) {
        ...
        for (t = 0; t < nthreads && i + t - nthreads < m; ++t) {
			...
			FILE *tpfile; // added by Son Nguyen
			tpfile = fopen("true_positive_stat.txt","a"); // added by Son Nguyen
			fprintf(tpfile,"Label path: %s\n",labelpath); // added by Son Nguyen
            ...
            int i;
            for (i = 0; i < nboxes; ++i) { // for each box detected

                int class_id;
                for (class_id = 0; class_id < classes; ++class_id) {  // for each class
                    float prob = dets[i].prob[class_id]; // get the probability of the class for this detection box
                    if (prob > 0) { // if the probability of that class is larger than zero 
                        detections_count++;
                        ...
                        int truth_index = -1;
                        float max_iou = 0;
						int label_bookmark = -1; // added by Son Nguyen
                        for (j = 0; j < num_labels; ++j) // for each label in the labels file
                        {
                            box t = { truth[j].x, truth[j].y, truth[j].w, truth[j].h }; // 
                            ...
                            if (current_iou > iou_thresh && class_id == truth[j].id) {
                                if (current_iou > max_iou) {
									...
									label_bookmark = j; // added by Son Nguyen
                                }
                            }
                        }
                        ...
                        if (prob > thresh_calc_avg_iou) {
                            ...
                            if (truth_index > -1 && found == 0) {
								if(label_bookmark > -1){ // added by Son Nguyen
									fprintf(tpfile,"TP box: %d %f %f %f %f\n",truth[label_bookmark].id,truth[label_bookmark].x,truth[label_bookmark].y,truth[label_bookmark].w,truth[label_bookmark].h); // added by Son Nguyen
								}
                                avg_iou += max_iou;
                                ++tp_for_thresh;
			...
			fclose(tpfile);	// added by Son Nguyen
